...
CRC.views.ShoppingCartView = Class.extend(CRC.util.Observable, {
    ...
    initialize: function() {
        this._shoppingItems = new CRC.model.ShoppingCartProducts();

        var me = this;
        $("#order").submit(function() {
            me.fireEvent('submitClicked', [me._shoppingItems.getJSONProducts()]);
    ...
    update: function() {
        ...
        if (this._shoppingItems.getProducts().length < 1) {
            cartModalItems.append($('<div></div>').text('Keine Waren im Korb').css('text-align', 'center'));
            return;
        }

        $.each(this._shoppingItems.getProducts(), function (index, item) {
        ...
        var value = $('<div></div>').addClass("sum large-6 columns align-right").text(this._shoppingItems.getSum() + " €");
    ...
    addProduct: function(product) {
        this._shoppingItems.add(product);
    },

    removeProduct: function(product) {
        var me = this;

        this._shoppingItems.remove(product, function(success) {
            if (success) {
